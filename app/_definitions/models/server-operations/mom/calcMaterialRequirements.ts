...
export default {
  ...
  async handler(ctx: ActionHandlerContext) {
    ...
    const scheduleItems = await server.queryDatabaseObject(`select * from mom_master_production_schedules where mrp_id = ANY($1::int[]);`, [[mrpId]]);
    ...
    const breakdowns = await server.queryDatabaseObject(`select * from mom_material_breakdowns where material_id = ANY($1::int[])`, [
      materials.map((item) => item.id),
    ]);

    const breakdownParts = await server.queryDatabaseObject(`select * from mom_material_breakdown_parts where breakdown_id = ANY($1::int[])`, [
