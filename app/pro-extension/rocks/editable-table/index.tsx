    ...
    [...(columns || []), ...removeColumns].forEach((col) => {
      ...
      tableColumns.push({
        ...
        render: (_, record, index) => {
          ...
          if (typeof col.control === "string") {
            const controlTrim = (col.control as string).trim();
            if (controlTrim.indexOf("function") === 0) {
              return new Function(`return ${controlTrim}`)()(record, index, (v: any) => {
                onFieldChange(record, col, index, v);
              });
            }

            return col.control;
          } else if (typeof col.control === "function") {
